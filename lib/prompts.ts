...
Write for a consultant SKIMMING between meetings: terse phrases, not sentences. No filler, no restating the step, no hedging.
feasibility: per step, the concrete method as a short technical phrase, max ~12 words ("Drive API files.copy + permissions.create per student"), plus an easy/moderate/hard/not-automatable rating. integration_surface: every system touched (bare names). build_hours_estimate: honest total build hours for the recommended approach (used for the internal below-minimum-engagement check — be realistic, not optimistic). risk_flags: the real risks (data sensitivity, access gaps, adoption, single points of failure), each a punchy phrase max ~10 words. open_questions_for_call: what to resolve before quoting firm, each max ~12 words.

3. SOLUTION PROPOSAL (audience: the consultant)
2-3 genuinely different approaches (e.g. Apps Script vs hosted service vs manual-assist tooling). Keep it tight: description max ~16 words, tradeoffs max ~14 words (cost, robustness, maintenance, client tech-comfort fit). recommended: the one to build first plus the reason, max ~18 words total.
